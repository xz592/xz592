...
if(!$action) {
	...
	$typeid = empty($typeid) || !in_array($typeid, array(1, 2, 3, 4, 5)) ? 1 : $typeid;
	...
	if(!submitcheck('editsubmit', 0, $seccodecheck)) {
		...
		if($typeid == 1) {
			...
			$membersql = "secques='$secquesnew', email='$emailnew' $newpasswdadd $authstradd1 $authstradd2";
	...
	if(submitcheck('spacesubmit')) {
		...
		if(!@file(DISCUZ_ROOT.'./mspace/'.$spacestyle.'/style.ini')) {
