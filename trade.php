...
if(!empty($orderid)) {
	...
	if(submitcheck('offlinesubmit') && in_array($offlinestatus, trade_offline($tradelog, 0))) {
		...
		if($offlinestatus == STATUS_SELLER_SEND) {
			...
			$db->query("UPDATE {$tablepre}trades SET lastbuyer='$tradelog[buyer]', lastupdate='$timestamp' WHERE tid='$tradelog[tid]' AND pid='$tradelog[pid]'");
			$db->query("UPDATE {$tablepre}trades SET amount=amount-$tradelog[number], totalitems=totalitems+1, tradesum=tradesum+$tradelog[price] WHERE tid='$tradelog[tid]' AND pid='$tradelog[pid]'", 'UNBUFFERED');
			$itemsubject = $tradelog['subject'];
		...
		$db->query("UPDATE {$tablepre}tradelog SET status='$offlinestatus', lastupdate='$timestamp' WHERE orderid='$orderid'");
	...
	if($action != 'trade' && !submitcheck('tradesubmit')) {
		...
		$db->query("UPDATE {$tablepre}trades SET lastbuyer='$discuz_user', lastupdate='$timestamp' WHERE tid='$trade[tid]' AND pid='$trade[pid]'");
