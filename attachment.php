...
if($attach['price'] && (!$discuz_uid || ($discuz_uid != $attach['uid'] && $adminid <= 0))) {
	$payrequired = 0;
	if(!$discuz_uid) {
		$payrequired = 1;
	} else {
		$query = $db->query("SELECT uid FROM {$tablepre}attachpaymentlog WHERE uid='$discuz_uid' AND aid='$attach[aid]'");
		$payrequired = !$db->result($query, 0);
	}
	if($payrequired) {
