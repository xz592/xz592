...
if(!empty($action) && !in_array($action, array('main', 'header', 'menu', 'illegallog', 'ratelog', 'modslog', 'medalslog', 'creditslog', 'banlog', 'cplog', 'errorlog'))) {
	...
	writelog('cplog', dhtmlspecialchars("$timestamp\t$discuz_userss\t$adminid\t$onlineip\t$action\t".($action == 'insenz' && in_array($operation, array('register', 'binding')) ? '' : $extra)));
}
...
if($action == 'menu') {
	...
	if($adminid == 1) {
		...
		if($action == 'home') {
			...
			$cpscript = 'ecommerce';
		} elseif($action == 'forumlinks' || $action == 'onlinelist' || $action == 'medals' || $action == 'censor' || $action == 'discuzcodes' || $action == 'smilies' || $action == 'icons' || $action == 'attachtypes' || $action == 'crons' || $action == 'creditslog' || $action == 'tags') {
			$cpscript = 'misc';
			...
			$cpscript = 'project';
		} elseif($action == 'threadtypes' || $action == 'typeoption' || $action == 'typedetail' || $action == 'optiondetail') {
			$cpscript = 'threadtypes';
		} elseif($action == 'videoconfig' || $action == 'video') {
			$cpscript = 'video';
		}
