...
if(empty($action) && $tid) {
	...
	if($qihoo_relate_bbsnum || $qihoo_relate_webnum || $insenz['topicrelatedad'] || ($insenz['traderelatedad'] && $thread['special'] == 2)) {
		$site = site();
		...
		if($db->num_rows($query)) {
			while($related = $db->fetch_array($query)) {
				...
				} elseif(($qihoo_relate_bbsnum || $qihoo_relate_webnum) && $related['type'] == 'general') {
	...
	if($authorid) {
		...
		$thread['replies'] = $db->result($query, 0);
	...
	$multipage = multi($thread['replies'] + 1, $ppp, $page, "viewthread.php?tid=$tid&amp;extra=$extra".(isset($highlight) ? "&amp;highlight=".rawurlencode($highlight) : '').(isset($authorid) ? "&amp;authorid=$authorid" : ''));
	...
	if(empty($postlist)) {
		...
		$seodescription = htmlspecialchars(strip_tags(cutstr($seodescription['message'], 150)));
	}
...
function viewthread_procpost($post, $special = 0) {
	...
	if($post['username']) {
		...
		if($showavatars) {
			if($_DCACHE['usergroups'][$post['groupid']]['groupavatar']) {
				$post['avatar'] = '<div class="avatar"><img src="'.$_DCACHE['usergroups'][$post['groupid']]['groupavatar'].'" border="0" alt="" /></div>';
			} elseif($_DCACHE['usergroups'][$post['groupid']]['allowavatar'] && $post['avatar']) {
				$post['avatar'] = '<div class="avatar"><img src="'.$post['avatar'].'" width="'.$post['avatarwidth'].'" height="'.$post['avatarheight'].'" border="0" alt="" /></div>';
			} else {
	...
	$post['message'] = discuzcode($post['message'], $post['smileyoff'], $post['bbcodeoff'], $post['htmlon'], $forum['allowsmilies'], $forum['allowbbcode'], ($forum['allowimgcode'] && $showimages ? 1 : 0), $forum['allowhtml'], ($forum['jammer'] && $post['authorid'] != $discuz_uid ? 1 : 0), 0, $post['authorid'], $forum['allowmediacode'], $post['pid']);
...
function viewthread_parsetags() {
	...
	if($firstpid && $tagstatus && !empty($_DCACHE['tags'])) {
		foreach($_DCACHE['tags'] as $tags) {
			$postlist[$firstpid]['message'] = preg_replace("/(?<=[\s\]>()]|[\x7f-\xff]|^)(".preg_quote($tags, '/').")(([.,:;-?!()\s\"<\[]|[\x7f-\xff]|$))/eiU", "'<span href=\"tag.php?name='.rawurlencode('\\1').'\" onclick=\"tagshow(event)\" class=\"t_tag\">\\1</span>\\2'", $postlist[$firstpid]['message'], 1);
		}
	}
}
