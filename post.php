...
if(empty($action)) {
	...
	$spp = $smcols * $smrows;
	$multipage = multi(count($_DCACHE['smilies_display']), $spp, $page, 'post.php?action=smilies&inajax=1', 0, 10, FALSE, TRUE);
	...
	include template('post_typeoption');
	exit;
...
$special = empty($special) || !is_numeric($special) || $special < 0 || $special > 5 ? 0 : intval($special);
